...
const hasRequiredDependentProperties = (value?: PublishCapabilityResult | null) =>
  Boolean(value?.properties?.some(property => property.required && property.dependent))
...
const singlePublishIssues = computed(() => {
  ...
  if (!schema.value) {
    ...
    if (hasRequiredDependentProperties(schema.value)) issues.push('仍有必填联动属性待加载')
...
const prepareBatch = async () => {
  ...
  for (const state of states) {
    try {
      ...
      if (!accountSchema || !supportsPublishForm(accountSchema.supportLevel) || !accountSchema.locationApiReady || hasRequiredDependentProperties(accountSchema)) {
    ...
    <section v-if="schema" class="publish-card">
      ...
      <div class="property-grid">
        <label v-for="property in schema.properties" :key="propertyKey(property)">
          ...
          <small v-else :class="{ 'optional-property': !property.required }">
            {{ property.required ? '必填联动选项尚未返回，请先完善上级属性。' : '平台未返回此可选字段，可留空继续发布。' }}
          </small>
        </label>
      </div>
    </section>
